...
namespace NzbDrone.Core.Applications.Mylar
{
    public class Mylar : ApplicationBase<MylarSettings>
    {
        ...
        private readonly ICached<List<MylarIndexer>> _schemaCache;
        private readonly IConfigFileProvider _configFileProvider;

        public Mylar(ICacheManager cacheManager, IMylarV3Proxy lidarrV1Proxy, IConfigFileProvider configFileProvider, IAppIndexerMapService appIndexerMapService, Logger logger)
            : base(appIndexerMapService, logger)
        {
            _schemaCache = cacheManager.GetCache<List<MylarIndexer>>(GetType());
        ...
        public override Dictionary<string, int> GetIndexerMappings()
        {
            ...
            {
                if ((string)indexer.Apikey == _configFileProvider.ApiKey)
                {
                    var match = AppIndexerRegex.Match((string)indexer.Host);

                    if (match.Groups["indexer"].Success && int.TryParse(match.Groups["indexer"].Value, out var indexerId))
                    {
                        //Add parsed mapping if it's mapped to a Indexer in this Prowlarr instance
                        mappings.Add($"{indexer.Type},{indexer.Name}", indexerId);
                    }
                }
            }
        ...
        public override void AddIndexer(IndexerDefinition indexer)
        {
            if (indexer.Capabilities.Categories.SupportedCategories(Settings.SyncCategories.ToArray()).Any())
            {
                ...
                _appIndexerMapService.Insert(new AppIndexerMap { AppId = Definition.Id, IndexerId = indexer.Id, RemoteIndexerId = $"{remoteIndexer.Type},{remoteIndexer.Name}" });
            }
        }
        ...
        public override void RemoveIndexer(int indexerId)
        {
            ...
            if (indexerMapping != null)
            {
                ...
                var indexerProps = indexerMapping.RemoteIndexerId.Split(",");
